...
class WpvTemplates {
		...
		if ( $total_pages > 1 ) {
			...
			echo paginate_links(
				array(
					...
					'total' => $total_pages,
					'prev_text' => __( 'Prev', 'fitness-wellness' ),
					'next_text' => __( 'Next', 'fitness-wellness' ),
				)
			);
			// xss ok
	...
	public static function pagination( $pagination_type = null ) {
		...
		if ( $pagination_type == 'paged' ) {
			...
			if ( (int)$max > (int)$paged ) {
				...
				echo '<div class="load-more"><a href="'.$url.'" class="'.$class.'"><span>'.__( 'Load more', 'fitness-wellness' ).'</span></a></div>';
			}
		}
	}
	...
	public static function has_post_siblings_buttons() {
		return is_singular( array( 'post', 'portfolio', 'tribe_organizer', 'tribe_venue' ) ) && current_theme_supports( 'wpv-ajax-siblings' ) && ! is_page_template( 'page-blank.php' );
	}
	...
	public static function custom_link_pages( $args = '' ) {
		$defaults = array(
			'before' => '<p id="post-pagination">' . __( 'Pages:', 'fitness-wellness' ),
			'after' => '</p>',
			'text_before' => '',
			'text_after' => '',
			'next_or_number' => 'number',
			'nextpagelink' => __( 'Next page', 'fitness-wellness' ),
			'previouspagelink' => __( 'Previous page', 'fitness-wellness' ),
			'pagelink' => '%',
			'echo' => 1,
		);
