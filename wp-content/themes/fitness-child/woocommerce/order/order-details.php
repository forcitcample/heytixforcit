...
?>
<h2 class="checkout_titles"><?php _e( 'Order Details', 'woocommerce' ); ?></h2>
<table class="shop_table order_details review_o">
	<thead>
	<tr>
		<th class="product-name"><?php _e( 'Product', 'woocommerce' ); ?></th>
		<th class="product-total a_right"><?php _e( 'Total', 'woocommerce' ); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php
		...
		foreach( $order->get_items() as $item_id => $item ) {
			...
			if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
				...
				<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
					...
					<td class="product-total a_right">
	...
	if ( $totals = $order->get_order_item_totals() ) {
		foreach ( $totals as $key => $total ) {
			...
			<tr>
				...
				<td class="a_right"><?php echo $value; ?></td>
			</tr>
			<?php
		}
	}
...
<header>
	<h2 class="checkout_titles"><?php _e( 'Customer Details', 'woocommerce' ); ?></h2>
</header>
<table class="shop_table shop_table_responsive customer_details">
	<?php
	if ( $order->billing_email ) {
		echo '<tr style="margin-bottom: 10px;"><th style="display: inline!important;">' . __( 'Email:', 'woocommerce' ) . '</th><td class="a_right" style="display: inline-block!important; float: right;">' . $order->billing_email . '</td></tr>';
	}

	if ( $order->billing_phone ) {
		echo '<tr><th style="display: inline!important;">' . __( 'Telephone:', 'woocommerce' ) . '</th><td class="a_right" style="display: inline-block!important; float: right; background-color: transparent;">' . $order->billing_phone . '</td></tr>';
	}
...
<div class="col2-set addresses">
	...
	<div class="col-1">
		...
		<header class="title">
			<h2 class="checkout_titles"><?php _e( 'Billing Address', 'woocommerce' ); ?></h2>
		</header>
		<address>
			<?php
	...
	<div class="col-2">
		...
		<header class="title">
			<h2 class="checkout_titles"><?php _e( 'Shipping Address', 'woocommerce' ); ?></h2>
		</header>
		<address>
			<?php
