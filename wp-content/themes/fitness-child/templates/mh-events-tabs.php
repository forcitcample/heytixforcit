<div class="ht-event-tabs">
    ...
    $terms = wp_get_post_terms($event_id, 'tribe_events_cat', $args );
        $is_blackout = get_post_meta($event_id, '_ecp_custom_3', true );
    //echo '<pre>';print_r($terms);
     $venueid=$terms['0']->term_id;//term_taxonomy_id;die;
    ...
    if ( $mh_tickets_enabled && class_exists( 'TribeWooTickets' ) ) {
        ob_start();
                ...
                $ticket_form->front_end_tickets_form(null);
    ...
    if($mh_guest_list_enabled){
        $guest_list_sc = '[tab title="Guest List" class="ht-event-tab-guest-list"]'. ht_guest_list_ticket() .'[/tab]';
    }
    // Get Table sc
    $table_form_sc = '';
    if(!empty($postids)){
  ...
  <select name="quantity" value="0" title="Qty" class="input-text qty text qbox"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option></select>
  </div>
</div>
...
// $raw.= 'Retail Price: $'.$price = get_post_meta($loop->post->ID, '_regular_price', true).'<br/><br/>';
    ...
    if($mh_get_tables_enabled){
               if(empty($is_blackout))
                   {
        $table_form_sc = '[tab title="Bottle Service" class="ht-event-tab-guest-list"]<div class="custom_package"><h2 class="custom_head">Looking for a Custom Package?</h2><span class="sub_heading_custom">Fill out form below to contact on of our VIP Hosts</span>[gravityform id="6" name="Bottle Service Request" title="false" description="false"]</div>[/tab]';
                }
    }
