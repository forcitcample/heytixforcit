...
// $raw.= 'Retail Price: $'.$price = get_post_meta($loop->post->ID, '_regular_price', true).'<br/><br/>';
    ...
    if($mh_get_tables_enabled){
        $table_form_sc = '[tab title="Bottle Service" class="ht-event-tab-guest-list"]'.$formbody.' <div class="custompackage"> <span class="mainheadinfor_custom">LOOKING FOR CUSTOM PACKAGE?</span><span class="sub_heading_custom">Fill out form below to contact on of our VIP Hosts</span>[gravityform id="6" name="Bottle Service Request" title="false" description="false"]</div>[/tab]';
    }
