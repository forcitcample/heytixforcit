...
// $raw.= 'Retail Price: $'.$price = get_post_meta($loop->post->ID, '_regular_price', true).'<br/><br/>';
    ...
    if($mh_get_tables_enabled){
        $table_form_sc = '[tab title="Bottle Service" class="ht-event-tab-guest-list"]<h2 class="preferred_head">HeyTix Preferred Packages</h2>'.$formbody.' <div class="bottle_clear"></div><div class="split_bar"></div><div class="custom_package"><h2 class="custom_head">Looking for a Custom Package?</h2><span class="sub_heading_custom">Fill out form below to contact on of our VIP Hosts</span>[gravityform id="6" name="Bottle Service Request" title="false" description="false"]</div>[/tab]';
    }
