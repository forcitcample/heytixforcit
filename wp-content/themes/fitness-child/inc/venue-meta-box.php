...
<?php
if ( $post->post_type == TribeEvents::VENUE_POST_TYPE ) {
    $venue_email_addresses = get_post_meta($postId, '_VenueEmailAddresses', false);

    if(empty($venue_email_addresses) || !is_array($venue_email_addresses)){
        $venue_email_addresses = array('');
    }

    $venue_email_addresses = array_values($venue_email_addresses);
    ?>
    <tr class="venue">
	<td style="vertical-align: top;padding-top: 10px;"><?php _e( 'Email Addresses:', 'tribe-events-calendar' ); ?></td>
	<td>
            <div class="venue-emails-container">
            <?php foreach ($venue_email_addresses as $venue_email){ ?>
                <p class="venue-email-adress-wrap"><input type="email" name="venue[VenueEmailAddresses][]" size="25" value="<?php echo esc_attr($venue_email); ?>" /> <span class="venu-email-delete dashicons dashicons-no"></span></p>
            <?php }?>
            </div>
            <span class="venue-email-add dashicons dashicons-plus"></span>
        </td>
    </tr>
<script type="text/javascript">
    (function($){
        $('body').on('click', '.venue-email-add', function(e){
            e.preventDefault();
            $(this).prev('.venue-emails-container').append('<p class="venue-email-adress-wrap"><input type="email" name="venue[VenueEmailAddresses][]" size="25" value="" /> <span class="venu-email-delete dashicons dashicons-no"></span></p>');
        });
        $('body').on('click', '.venu-email-delete', function(e){
            e.preventDefault();
            if($(this).closest('.venue-emails-container').find('.venue-email-adress-wrap').length > 1){
                $(this).closest('.venue-email-adress-wrap').remove();
            }
        });
    })(jQuery);
</script>
