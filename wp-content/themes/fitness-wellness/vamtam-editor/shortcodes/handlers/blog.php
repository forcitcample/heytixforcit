...
class WPV_Blog {
	...
	public static function shortcode($atts, $content = null, $code = 'blog') {
		...
		$image_sizes = array( 'post-loop', 'post-loop@2x', 'full' );

		if ( $news ) {
			$thumb_prefix =	$layout === 'masonry' && ! has_post_format( 'gallery' ) ? 'portfolio-masonry' : 'post-small';

			$image_sizes = array(
				"{$thumb_prefix}-1", "{$thumb_prefix}-2", "{$thumb_prefix}-3", "{$thumb_prefix}-4",
				"{$thumb_prefix}-1@2x", "{$thumb_prefix}-2@2x", "{$thumb_prefix}-3@2x", "{$thumb_prefix}-4@2x",
			);
		}
