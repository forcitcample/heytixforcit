...
class wpv_posts extends WP_Widget {
	...
	public function __construct() {
		$widget_options = array(
			'classname' => 'wpv_posts',
			'description' => __("Displays a list of posts/comments", 'fitness-wellness')
		);
		parent::__construct('wpv_posts', __('Vamtam - Multi widget', 'fitness-wellness') , $widget_options);
	...
	private function get_section_title($orderby, $single=false) {
		if($orderby == 'comment_count')
			return apply_filters('wpv_multiwidget_tab_title', __('Popular', 'fitness-wellness'), $orderby, $single);
		if($orderby == 'date')
			return apply_filters('wpv_multiwidget_tab_title', __('Newest', 'fitness-wellness'), $orderby, $single);
		if($orderby == 'comments')
			return apply_filters('wpv_multiwidget_tab_title', __('Comments', 'fitness-wellness'), $orderby, $single);
		if($orderby == 'tags')
			return apply_filters('wpv_multiwidget_tab_title', __('Tags', 'fitness-wellness'), $orderby, $single);
	}
