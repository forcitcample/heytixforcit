...
class WpvAdmin {
	...
	public static function actions() {
		self::$option_pages = array(
			'general' => array(
				__( 'VamTam | General Settings', 'fitness-wellness' ),
				__( 'General Settings', 'fitness-wellness' ),
			),

			'layout' => array(
				__( 'VamTam | Layout', 'fitness-wellness' ),
				__( 'Layout', 'fitness-wellness' ),
			),

			'styles' => array(
				__( 'VamTam | Styles', 'fitness-wellness' ),
				__( 'Styles', 'fitness-wellness' ),
			),

			'import' => array(
				__( 'VamTam | Quick Import', 'fitness-wellness' ),
				__( 'Quick Import', 'fitness-wellness' ),
			),

			'help' => array(
				__( 'VamTam | help', 'fitness-wellness' ),
				__( 'Help', 'fitness-wellness' ),
			),
		);
	...
	public static function update_warning() {
		if ( did_action( 'load-update-core.php' ) ) {
			...
			_e( 'Hey, just a polite reminder that if you update WordPress you will also need to update your theme and plugins.', 'fitness-wellness' );
			echo '</strong>'; // xss ok
			echo '</p><p>'; // xss ok
			printf( __( 'You should see any available theme updates on this page if you have entered your purchase information in <a href="%s">VamТam/General</a>', 'fitness-wellness' ), esc_url( admin_url( 'admin.php?page=wpv_general#purchase-tab-3' ) ) );
			echo '</p><p>'; // xss ok
			_e( "Please note that if we haven't released an update, you shouldn't update your WordPress and plugins until we release one. Otherwise you may run into various compatibility issues.", 'fitness-wellness' );
			echo '</p><p>'; // xss ok
			printf( __( 'If you are unsure as to whether it is safe to update your site, please do ask us at <a href="%s">%s</a> and we\'ll help you.', 'fitness-wellness' ), esc_url( 'http://support.vamtam.com' ), 'our support site' );
			echo '</p></div>'; // xss ok;
		}
	}
	...
	public static function icons_selector() {
		...
		<div class="wpv-config-icons-selector hidden">
			<input type="search" placeholder="<?php esc_attr_e( 'Filter icons', 'fitness-wellness' ) ?>" class="icons-filter"/>
	...
	public static function load_metaboxes( $post_id = null ) {
		$config = array(
			...
			'title' => __( 'VamTam Testimonials', 'fitness-wellness' ),
		...
		$config = array(
			...
			'title' => __( 'VamTam Post Formats', 'fitness-wellness' ),
			'pages' => array( 'post' ),
			'context' => 'normal',
			'priority' => 'high',
			'post_id' => $post_id,
		...
		$config = array(
			...
			'title' => __( 'VamTam Portfolio Formats', 'fitness-wellness' ),
		...
		$config = array(
			...
			'title' => __( 'VamTam Portfolio Format', 'fitness-wellness' ),
			'pages' => array( 'portfolio' ),
			'context' => 'side',
			'priority' => 'high',
			'post_id' => $post_id,
		...
		$config = array(
			...
			'title' => __( 'VamTam Options', 'fitness-wellness' ),
