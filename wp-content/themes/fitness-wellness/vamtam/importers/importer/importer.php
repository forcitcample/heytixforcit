...
class WPV_Import extends WP_Importer {
	...
	function import_end() {
		...
		echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', 'wordpress-importer' ) . '</p>'; // xss ok
...
<?php
	...
	function author_select( $n, $author ) {
		...
		if ( $create_users ) {
			...
			echo ' <input type="text" name="' . esc_attr( 'user_new[' . $n . ']' ) . '" value="'. esc_attr( $value ) .'" /><br />';
		}
		...
		echo '<input type="hidden" name="' . esc_attr( 'imported_authors[' . $n . ']' ) . '" value="' . esc_attr( $author['author_login'] ) . '" />';
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . $id->get_error_message(); // xss ok
				}
				echo '<br />';
				continue;
			}
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( strstr( $domain, 'pa_' ) && wpv_has_woocommerce() ) {
	...
	function header() {
		echo '<div class="wrap">';
		echo '<h2>' . __( 'Import WordPress', 'wordpress-importer' ) . '</h2>'; // xss ok
	...
	function greet() {
		...
		echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'wordpress-importer' ).'</p>'; // xss ok
		echo '<p>'.__( 'Choose a WXR file to upload, then click Upload file and import.', 'wordpress-importer' ).'</p>'; // xss ok
