...
class WooCommerce_Order_Barcodes {
	...
	public function __construct ( $file = '', $version = '1.0.0' ) {
		...
		add_action( 'init', array( $this, 'get_barcode_image' ), 1 );
...
<?php
	...
	public function display_barcode ( $order_id = 0, $before = '', $after = '', $echo = true ) {
		...
		$barcode = $before . '<img src="' . esc_url( $barcode_url ) . '" title="' . __( 'Barcode', 'woocommerce-order-barcodes' ) . '" alt="' . __( 'Barcode', 'woocommerce-order-barcodes' ) . '" style="display:inline;border:0;" /><br/><span style="color:' . $this->barcode_colours['foreground'] . ';font-family:monospace;">' . $barcode_text . '</span>' . $after;
	...
	public function barcode_url ( $order_id = 0 ) {
		...
		return trailingslashit( get_site_url() ) . '?wc_barcode=' . $order_id;
	...
	public function get_barcode_image () {
		if( isset( $_GET['wc_barcode'] ) && 0 < intval( $_GET['wc_barcode'] ) ) {
	    ...
	    $html = '<div class="woocommerce">';
			...
			$html .= '<div id="barcode-scan-form">
						<form name="barcode-scan" action="" method="post">
							<select name="scan-action" id="scan-action" class="scan_action">
							...
							<input type="text" name="scan-code" id="scan-code" value="" placeholder="' . __( 'Scan or enter barcode', 'woocommerce-order-barcodes' ) . '" />
	...
	/**
	 * Plugin installation - runs on activation
	 * @access  public
	 * @since   1.0.0
	 * @return  void
	 */
