...
<?php do_action( 'woocommerce_email_header', $email_heading ); ?>
	...
	<table class="sales_report">
		<tr>
			<th class="venue" colspan="4">SUMMARY</th>
		</tr>
		<?php
		$total_rows = count($rows)-1;
		for( $x=0; $x <= $total_rows; $x++ ) : ?>
			<tr class="info_<?php echo (($x % 2) == 0) ? 'w' : 'g'; ?>">
				<td style="width: 50%; text-align: left;<?php echo ($x == 3) ? ' font-weight: bold;' : ''; ?>"><?php echo $rows[$x]->get_label(); ?></td>
				<td style="width: 50%; text-align: right;<?php echo ($x == 3) ? ' font-weight: bold;' : ''; ?>"><?php echo $rows[$x]->get_value(); ?></td>
			</tr>
		<?php endfor; ?>
	</table>
...
<?php if(count($venues) > 0) :
	...
	<table class="sales_report">
	<?php foreach ( $venues as $venue_id => $events) :
		$events = array_values($events);
	?>
		<tr>
			<th class="venue" colspan="4"><?php echo strtoupper($events[0]['venue_name']); ?></th>
		</tr>
		<?php
		$rows = count($events)-1;
