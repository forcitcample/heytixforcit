...
class userpro_api {
	...
	******************************************/
	function twitter_redirect_url(){
		$curent_page_url = remove_query_arg( array( 'oauth_token', 'oauth_verifier' ), esc_url($this->get_current_page_url()) );
	...
	******************************************/
	function accept_invite_to_verify($user_id) {
		...
		if ( $salt != '' && strlen($salt) == 20 && $this->user_exists($user_id) ){
			$url = home_url() . '/';
			$url = add_query_arg( 'act', 'verified_invitation', esc_url($url) );
			$url = add_query_arg( 'user_id', $user_id, esc_url($url) );
			$url = add_query_arg( 'hash_secret', $salt, esc_url($url) );
			return $url;
		}
	}
