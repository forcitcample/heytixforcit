...
$uppayment=get_option('userpro_payment');
			...
			case 'register':
				...
				if ( !isset($user_exists) and email_exists($form['user_email']) == false ) {
	...
	function userpro_invite_mail_send($users){
		...
		foreach ($users as $user) {
		   if( email_exists( $user ) == false) {
			...
			wp_mail($user,'You are invited to register at '.get_bloginfo('name'), html_entity_decode($message) , $headers);
