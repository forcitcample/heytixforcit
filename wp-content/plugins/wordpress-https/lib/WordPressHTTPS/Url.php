	...
	public static function fromString( $string ) {
		...
		if ( isset($url_parts[1][0]) ) {
			if ( $url_parts = parse_url( $url_parts[1][0] ) ) {
				$url->setScheme((isset($url_parts['scheme']) && array_key_exists('scheme', $url_parts)) ? $url_parts['scheme'] : '');
				$url->setUser((isset($url_parts['user']) && array_key_exists('user', $url_parts)) ? $url_parts['user'] : '');
				$url->setPass((isset($url_parts['pass']) && array_key_exists('pass', $url_parts)) ? $url_parts['pass'] : '');
				$url->setHost((isset($url_parts['host']) && array_key_exists('host', $url_parts)) ? $url_parts['host'] : '');
				$url->setPort((isset($url_parts['port']) && array_key_exists('port', $url_parts)) ? $url_parts['port'] : '');
				$url->setPath((isset($url_parts['path']) && array_key_exists('path', $url_parts)) ? $url_parts['path'] : '');
				$url->setQuery((isset($url_parts['query']) && array_key_exists('query', $url_parts)) ? $url_parts['query'] : '');
				$url->setFragment((isset($url_parts['fragment']) && array_key_exists('fragment', $url_parts)) ? $url_parts['fragment'] : '');
				return $url;
			}
		} else {
			 return false;
