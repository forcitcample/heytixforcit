jQuery( function() {
	...
	jQuery( 'form.checkout' ).on( 'checkout_place_order_heytix-stripe-connect', function() {
	...
	function stripeFormHandler() {
		...
		if ( ! jQuery( '#payment_method_heytix-stripe-connect' ).is( ':checked' ) ) {
			return true;
		}
		...
		if ( jQuery( 'input#terms' ).size() === 1 && jQuery( 'input#terms:checked' ).size() === 0 ) {
			alert( heytix_stripe_connect_params.i18n_terms );

			return false;
		}

		if ( jQuery( '#createaccount' ).is( ':checked' ) && jQuery( '#account_password' ).length && jQuery( '#account_password' ).val() === '' ) {
			alert( heytix_stripe_connect_params.i18n_required_fields );
		...
		StripeCheckout.open({
			key:         heytix_stripe_connect_params.key,
			address:     false,
