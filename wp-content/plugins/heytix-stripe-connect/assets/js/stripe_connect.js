Stripe.setPublishableKey( heytix_stripe_connect_params.key );

jQuery( function() {

	/* Checkout Form */
	jQuery('form.checkout').on('checkout_place_order_heytix-stripe-connect', function( event ) {
...
function stripeFormHandler() {
	if ( jQuery('#payment_method_stripe').is(':checked') && ( jQuery('input[name=stripe_card_id]:checked').size() == 0 || jQuery('input[name=stripe_card_id]:checked').val() == 'new' ) ) {
		if ( jQuery( 'input.stripe_token' ).size() == 0 ) {
			...
			} else if ( heytix_stripe_connect_params.billing_first_name ) {
				data.name = heytix_stripe_connect_params.billing_first_name + ' ' + heytix_stripe_connect_params.billing_last_name;
			}
			...
			if ( jQuery('#billing_address_1').size() > 0 ) {
				...
				data.address_line1   = heytix_stripe_connect_params.billing_address_1;
				data.address_line2   = heytix_stripe_connect_params.billing_address_2;
				data.address_state   = heytix_stripe_connect_params.billing_state;
				data.address_city    = heytix_stripe_connect_params.billing_city;
				data.address_zip     = heytix_stripe_connect_params.billing_postcode;
				data.address_country = heytix_stripe_connect_params.billing_country;
			}
