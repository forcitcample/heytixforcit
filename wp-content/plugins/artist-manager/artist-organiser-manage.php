...
function eventorganiser_event_add_columns( $columns ) {
	...
	$columns['title'] = __( 'Artist', 'eventorganiser' );
...
function eventorganiser_restrict_events_by_category() {
	...
	$category_tax = get_taxonomy( 'artist-category' );
	
	if( 'event' == $typenow && $category_tax && wp_count_terms( 'artist-category' ) > 0 ){
		eo_taxonomy_dropdown( array( 
			'taxonomy'        => 'artist-category',
			'selected'        => get_query_var( 'artist-category' ),
			'hide_empty'      => false, 
...
function eventorganiser_restrict_events_by_venue() {
	...
	$venue_tax = get_taxonomy( 'artist-venue' );
	
	//Only add if CPT is event
	if( 'event' == $typenow && $venue_tax && wp_count_terms( 'artist-venue' ) > 0  ){
		eo_taxonomy_dropdown( array(
				'taxonomy'        => 'artist-venue',
				'selected'        => get_query_var( 'artist-venue' ),
				'hide_empty'      => false,
...
function eventorganiser_display_occurrences() {
	...
	if ( $typenow == 'event' ):
		$intervals = array(
			'all' => __( 'View all artist', 'eventorganiser' ),
			'future' => __( 'Future artist', 'eventorganiser' ),
			'expired' => __( 'Expired artist', 'eventorganiser' ),
			'P1D' => __( 'artist within 24 hours', 'eventorganiser' ),
			'P1W' => __( 'artist within 1 week', 'eventorganiser' ),
			'P2W' => sprintf( __( 'artist within %d weeks', 'eventorganiser' ), 2 ),
			'P1M' => __( 'artist within 1 month', 'eventorganiser' ),
			'P6M' => sprintf( __( 'artist within %d months', 'eventorganiser' ), 6 ),
			'P1Y' => __( 'artist within 1 year', 'eventorganiser' ),
		);
