...
function eventorganiser_create_event_taxonomies() {
	...
	if( !eventorganiser_get_option( 'prettyurl' ) ){
		...
		$cat_slug = trim(eventorganiser_get_option('url_cat','artist/category'), "/");
		$cat_rewrite = array( 'slug' => $cat_slug, 'with_front' => false );

		$tag_slug = trim(eventorganiser_get_option('url_tag','artist/tag'), "/");
		$tag_rewrite = array( 'slug' => $tag_slug, 'with_front' => false );

		$venue_slug = trim(eventorganiser_get_option('url_venue','artist/venue'), "/");
	...
	$category_labels = array(
		'name'              => __( 'Artist Categories', 'eventorganiser' ),
	...
	if( eventorganiser_get_option('eventtag') ){
		...
		$tag_labels = array(
			'name'                       => __( 'Artist Tags', 'eventorganiser' ),
	...
	if( $event_tag_args ){
		register_taxonomy( 'event-tag',array( 'artist' ), $event_tag_args );	
	}
...
function eventorganiser_cpt_register() {
	...
	$labels = array(
		'name'               => __( 'Artist', 'eventorganiser' ),
		'singular_name'      => __( 'Artist', 'eventorganiser' ),
		'add_new'            => _x( 'Add New', 'post', 'eventorganiser' ),
		'add_new_item'       => __( 'Add New Artist', 'eventorganiser' ),
		'edit_item'          => __( 'Edit Artist', 'eventorganiser' ),
		'new_item'           => __( 'New Artist', 'eventorganiser' ),
		'all_items'          => __( 'All Artist', 'eventorganiser' ),
		'view_item'          => __( 'View Artist', 'eventorganiser' ),
		'search_items'       => __( 'Search artist', 'eventorganiser' ),
		'not_found'          => __( 'No artist found', 'eventorganiser' ),
		'not_found_in_trash' => __( 'No artist found in Trash', 'eventorganiser' ),
		'parent_item_colon'  => '',
		'menu_name'          => __( 'Artist', 'eventorganiser' ),
	);
...
function eventorganiser_messages( $messages ) {
	...
	$messages['event'] = array(
		...
		1 => sprintf( __('Artist updated. <a href="%s">View artist</a>','eventorganiser'), esc_url( get_permalink($post_ID) ) ),
		2 => __('Custom field updated.'),
		3 => __('Custom field deleted.'),
		4 => __('Artist updated.','eventorganiser'),
		/* translators: %s: date and time of the revision */
		5 => isset($_GET['revision']) ? sprintf( __('Artist restored to revision from %s','eventorganiser'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		6 => sprintf( __('Artist published. <a href="%s">View artist</a>','eventorganiser'), esc_url( get_permalink($post_ID) ) ),
		7 => __('Event saved.'),
		8 => sprintf( __('Artist submitted. <a target="_blank" href="%s">Preview artist</a>','eventorganiser'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
		9 => sprintf( __('Artist scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview artist</a>','eventorganiser'),
		 // translators: Publish box date format, see https://php.net/date
      		date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
		10 => sprintf( __('Artist draft updated. <a target="_blank" href="%s">Preview artist</a>','eventorganiser'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
		20 => __('This event has been broken from a recurring artist.','eventorganiser')
  	);
	return $messages;
}
