...
class FUE_Woocommerce {
    ...
    public function __construct() {
        ...
        add_filter( 'fue_email_query', array($this, 'email_query'), 10, 3 );
        ...
        // email forms
        add_action( 'fue_email_form_before_subject', array($this, 'email_form_options') );
        ...
        //add_action( 'fue_email_created', array($this, 'fork_queue_old_orders'), 10, 2 );
        ...
        // queue old orders
        //add_action( 'fue_queue_old_orders', array($this, 'queue_old_orders') );
    ...
    public function trigger_types( $triggers = array(), $email_type = '' ) {
        ...
        if ( WC_FUE_Compatibility::is_wc_version_gt('2.2') ) {
            $statuses = wc_get_order_statuses();

            foreach ( $statuses as $key => $status ) {
                $key = str_replace( 'wc-', '', $key );
                $triggers[ $key ] = sprintf(__('after Order Status: %s', 'follow_up_emails'), $status );
            }
        } else {
        ...
        if ( WC_FUE_Compatibility::is_wc_version_gte_2_2() ) {
            ...
            $triggers['refund_manual']      = __('after refunded manually', 'follow_up_emails');
            $triggers['refund_successful']  = __('after refunded successfully', 'follow_up_emails');
            $triggers['refund_failed']      = __('after refund failed', 'follow_up_emails');
        }
    ...
    public function email_type_triggers( $type_triggers ) {
        ...
        $type_triggers['generic']   = array('first_purchase', 'cart', 'refund_manual', 'refund_successful', 'refund_failed');
        ...
        if ( WC_FUE_Compatibility::is_wc_version_gte_2_2() ) {
            ...
            foreach ( $statuses as $key => $status ) {
                $order_triggers[] = str_replace( 'wc-', '', $key );
            }

            $type_triggers['reminder']  = array('wc-processing', 'wc-completed');
        } else {
            $order_statuses = (array)get_terms( 'shop_order_status', array('hide_empty' => 0, 'orderby' => 'id') );
    ...
    public function email_query( $results, $term, $all_emails = array() ) {
        global $wpdb, $woocommerce;
    ...
    public function email_interval_meta( $values ) {
        ...
        <span class="show-if-order_total_above hide-if-date order_total_above_span hideable">
            ...
            <input type="text" name="meta[order_total_above]" id="order_total_above" value="<?php if (isset($values['meta']['order_total_above'])) echo $values['meta']['order_total_above']; ?>" />
        </span>

        <span class="show-if-order_total_below hide-if-date order_total_below_span hideable">
            <?php echo get_woocommerce_currency_symbol(); ?>
            <input type="text" name="meta[order_total_below]" id="order_total_below" value="<?php if (isset($values['meta']['order_total_below'])) echo $values['meta']['order_total_below']; ?>" />
        </span>
        ...
        <span class="show-if-total_purchases hide-if-date total_purchases_span hideable">
            ...
            <select name="meta[total_purchases_mode]">
                <option value="equal to" <?php if (isset($values['meta']['total_purchases_mode']) && $values['meta']['total_purchases_mode'] == 'equal to') echo  'selected'; ?>><?php _e('equal to', 'follow_up_emails'); ?></option>
                <option value="greater than" <?php if (isset($values['meta']['total_purchases_mode']) && $values['meta']['total_purchases_mode'] == 'greater than') echo  'selected'; ?>><?php _e('greater than', 'follow_up_emails'); ?></option>
            </select>

            <?php echo get_woocommerce_currency_symbol(); ?>
            <input type="text" name="meta[total_purchases]" value="<?php if (isset($values['meta']['total_purchases'])) echo $values['meta']['total_purchases']; ?>" />
        </span>

        <span class="show-if-total_orders hide-if-date total_orders_span hideable">
            <span class="description"><?php _e('is', 'follow_up_emails'); ?></span>
            <select name="meta[total_orders_mode]">
                <option value="equal to" <?php if (isset($values['meta']['total_orders_mode']) && $values['meta']['total_orders_mode'] == 'equal to') echo  'selected'; ?>><?php _e('equal to', 'follow_up_emails'); ?></option>
                <option value="greater than" <?php if (isset($values['meta']['total_orders_mode']) && $values['meta']['total_purchases_mode'] == 'greater than') echo  'selected'; ?>><?php _e('greater than', 'follow_up_emails'); ?></option>
            </select>

            <input type="text" name="meta[total_orders]" value="<?php if (isset($values['meta']['total_orders'])) echo $values['meta']['total_orders']; ?>" />
        </span>
    <?php
    }

    public function email_form_options( $values ) {
        ...
        <input type="checkbox" name="meta[remove_signup_emails_on_purchase]" id="remove_signup_emails_on_purchase" value="yes" <?php if (isset($values['meta']['remove_signup_emails_on_purchase']) && $values['meta']['remove_signup_emails_on_purchase'] == 'yes') echo 'checked'; ?> />
        <label for="remove_signup_emails_on_purchase" class="inline">
            <?php _e('Unsubscribe customers from this email once they have made a purchase', 'follow_up_emails'); ?>
            <img class="help_tip" title="<?php _e('Enabling this option will stop customers from getting this email once they have made a purchase.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL .'/images/help.png'; ?>" width="16" height="16" style="float:none;" />
        </label>
    <?php
    }
    ...
    /**
     * Process the importing of initial orders
     *
     * This method imports 500 orders on every page load
     * until there are no more orders left to import
     *
     */
    ...
    function cart_updated() {
        ...
        if ( !$email_created ) {
            ...
            foreach ( $cart['cart'] as $item_key => $item ) {
                ...
                $email = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE `interval_type` = 'cart' AND `category_id` IN (". $ids .") ORDER BY `priority` ASC");
                ...
                foreach ( $email as $e ) {
                    ...
                    if ( $check == 0 && !in_array($e->id .'_'. $item['product_id'], $cart_session) ) {
                        ...
                        $emails[] = array('id' => $e->id, 'item' => $item['product_id'], 'priority' => $e->priority);
                    }
                }
            }
    ...
    public function refund_manual( $refund_id ) {
        ...
        $emails = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'refund_manual' AND status = ". FUE::STATUS_ACTIVE);
    ...
    public function refund_processed( $refund, $successful ) {
        ...
        $emails = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = '{$trigger}' AND status = ". FUE::STATUS_ACTIVE);
    ...
    public static function create_email_orders( $triggers, $order_id = '' ) {
        ...
        $order          = ($order_id > 0) ? WC_FUE_Compatibility::wc_get_order($order_id) : false;
        ...
        if ( $order ) {
            ...
            if ( WC_FUE_Compatibility::get_order_user_id( $order ) > 0 ) {
                $fue_customer_id = $wpdb->get_var( $wpdb->prepare("SELECT id FROM {$wpdb->prefix}followup_customers WHERE user_id = %d", WC_FUE_Compatibility::get_order_user_id( $order ) ) );
            } else {
        ...
        foreach ( $items as $item ) {
            ...
            if ( $email_results ) {
                foreach ($email_results as $email) {
                    ...
                    if ( $email->interval_type == 'date' && FUE::send_date_passed( $email->id ) ) {
                        continue;
                    }
            ...
            foreach ( $results as $row ) {
                ...
                // skip date emails that have passed
                if ( $row->interval_type == 'date' && FUE::send_date_passed( $row->id ) ) {
                    continue;
                }
        ...
        if ( !empty($always_prods) ) {
            foreach ( $always_prods as $row ) {
                $email              = $wpdb->get_row( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE `id` = %d AND status = ". FUE::STATUS_ACTIVE, $row['id']) );
                $interval           = (int)$email->interval_num;
                $interval_duration  = $email->interval_duration;

                $product_id = ($row['parent'] && $row['parent'] > 0) ? $row['parent'] : $row['item'];
                ...
                if ( $reminder_count == 0 && $email->email_type == 'reminder') {
                    ...
                    foreach ( $items as $item ) {
                        ...
                        if ( $item_id == $row['item'] || $variation_id == $row['item'] ) {
                            $qty = $item['qty'];
                            ...
                            if ( isset($item['item_meta']) && !empty($item['item_meta']) ) {
                                ...
                                foreach ( $item['item_meta'] as $meta ) {
                                    if ($wc2) {
                                        ...
                                        } elseif ( isset($item['filterscase']) ) {
                                            $num_products = $item['filterscase'];
                                        }
                                    } else {
                    ...
                    $lifespan = get_post_meta( $product_id, 'filter_lifespan', true );
            ...
            if ( $top !== false ) {
                ...
                foreach ( $emails as $prod_email ) {
                    ...
                    if ( $prod_email['item'] == $top['item'] ) {
                        ...
                        $email      = $wpdb->get_row( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE `id` = %d AND status = ". FUE::STATUS_ACTIVE, $prod_email['id']) );
                        $interval   = (int)$email->interval_num;
        ...
        if ( !$order_created ) {
            ...
            foreach ( $items as $item ) {
                ...
                $email = $wpdb->get_results("SELECT DISTINCT `id`, `priority` FROM {$wpdb->prefix}followup_emails WHERE `interval_type` IN ($trigger) AND `product_id` = 0 AND `category_id` > 0 AND `category_id` IN (". $ids .") AND `email_type` <> 'generic' AND `always_send` = 0 AND status = ". FUE::STATUS_ACTIVE ." ORDER BY `priority` ASC");
                ...
                if ( $top !== false ) {
                    ...
                    foreach ( $emails as $cat_email ) {
                        ...
                        if ( $cat_email['item'] == $top['item'] ) {
                            ...
                            // if schedule is within 60 minutes, add to queue
                            $email      = $wpdb->get_row( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE `id` = %d AND status = ". FUE::STATUS_ACTIVE, $cat_email['id']) );
                            $interval   = (int)$email->interval_num;
        ...
        if ( !$order_created && !defined('FUE_ORDER_CREATED') ) {
            // find a generic mailer
            $emails         = $wpdb->get_results("SELECT DISTINCT * FROM {$wpdb->prefix}followup_emails WHERE `email_type` = 'generic' AND `interval_type` IN ($trigger) AND status = ". FUE::STATUS_ACTIVE ." ORDER BY `priority` ASC");
        ...
        if ( $order !== false && WC_FUE_Compatibility::get_order_status( $order ) == 'completed' ) {
            /**
            ...
            if ( $fue_customer_id ) {
                ...
                foreach ( $item_ids as $item_id ) {
                    ...
                    if ( $count == 1 ) {
                        ...
                        $emails = $wpdb->get_results( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'first_purchase' AND email_type = 'normal' AND product_id = %d AND status = ". FUE::STATUS_ACTIVE, $item_id) );
                        ...
                        $emails = $wpdb->get_results( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'product_purchase_above_one' AND email_type = 'normal' AND product_id = %d AND status = ". FUE::STATUS_ACTIVE, $item_id) );
                    ...
                    if ( $cat_ids ) {
                        foreach ( $cat_ids as $cat_id ) {
                            ...
                            if ( $count == 1 ) {
                                ...
                                $emails = $wpdb->get_results( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'first_purchase' AND email_type = 'normal' AND category_id = %d AND status = ". FUE::STATUS_ACTIVE, $cat_id) );
                                ...
                                $emails = $wpdb->get_results( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'product_purchase_above_one' AND email_type = 'normal' AND category_id = %d AND status = ". FUE::STATUS_ACTIVE, $cat_id) );
                ...
                if ( $count == 1 ) {
                    // first time ordering
                    $emails = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}followup_emails WHERE interval_type = 'first_purchase' AND email_type = 'generic' AND status = ". FUE::STATUS_ACTIVE);
            ...
            $emails = $wpdb->get_results("SELECT DISTINCT * FROM {$wpdb->prefix}followup_emails WHERE `email_type` = 'customer' AND `interval_type` IN ($trigger) AND status = ". FUE::STATUS_ACTIVE ." ORDER BY `priority` ASC");
            ...
            foreach ( $emails as $email ) {
                ...
                if ( $email->interval_type == 'order_total_above' ) {
                    ...
                    if ( WC_FUE_Compatibility::get_order_user_id( $order ) > 0 ) {
                        $num_orders = $wpdb->get_var( $wpdb->prepare("SELECT total_orders FROM {$wpdb->prefix}followup_customers WHERE user_id = %d", WC_FUE_Compatibility::get_order_user_id( $order ) ) );
                    } else {
                        $num_orders = $wpdb->get_var( $wpdb->prepare("SELECT total_orders FROM {$wpdb->prefix}followup_customers WHERE email_address = %s", $order->billing_email) );
                    }
        ...
        if ( $order && ( $order_status == 'processing' || $order_status == 'completed' ) ) {
            ...
            $emails = $wpdb->get_results("SELECT DISTINCT * FROM {$wpdb->prefix}followup_emails WHERE `email_type` = 'customer' AND `interval_type` = 'after_last_purchase' AND status = ". FUE::STATUS_ACTIVE ." ORDER BY `priority` ASC");
            ...
            foreach ( $emails as $email ) {
                ...
                if ( $order_user_id > 0 ) {
                    $last_order_date    = $wpdb->get_var( $wpdb->prepare("SELECT p.post_date FROM {$wpdb->posts} p, {$wpdb->prefix}followup_customer_orders co WHERE co.followup_customer_id = %d AND co.order_id = p.ID AND p.post_status = 'publish' ORDER BY p.ID DESC LIMIT 1", $order_user_id) );
                } else {
    ...
    public function send_email_data( $email_data, $email_order, $email_row ) {
        ...
        if ( $email_order->order_id != 0 ) {
            ...
            if ( WC_FUE_Compatibility::is_wc_version_gte_2_2() ) {
                $order_statuses_array = wc_get_order_statuses();
            ...
            if (in_array($email_row->interval_type, $all_statuses) && $email_row->interval_type !== $order_status) {
    ...
    public function test_email_form( $values ) {
        ...
        if ($values['type'] == 'generic') {
            ...
            ?>
            <select id="test_type">
                <option value="product" selected><?php _e('a Product', 'follow_up_emails'); ?></option>
                <option value="order"><?php _e('an Order', 'follow_up_emails'); ?></option>
            </select>

            <span id="test_email_order">
                <?php _e( 'with Order Number: ', 'follow_up_emails' ); ?>
                <input type="number" id="order_id" placeholder="e.g. 105" size="5" class="test-email-field" />
            </span>

            <span id="test_email_product">
                <select class="product-id-chzn" data-placeholder="Select a Product" style="width: 200px;" multiple></select>
                <input type="hidden" id="product_id" class="test-email-field" />
            </span>
            ...
            jQuery("#test_type").change(function() {
                if (jQuery(this).val() == "order") {
                    jQuery("#test_email_order").show();
                    jQuery("#test_email_product").hide();
                } else {
                    jQuery("#test_email_product").show();
                    jQuery("#test_email_order").hide();
                }
            }).change();
    ...
    <?php
        ...
        if ( in_array( $defaults['interval_type'], array('refund_manual', 'refund_successful', 'refund_failed') ) ):
            ?>
    ...
    public function email_variables( $vars, $email_data, $email_order, $email_row ) {
        ...
        switch ( $email_type ) {
            case 'generic':
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{order_billing_address}', '{order_shipping_address}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{item_names}', '{item_categories}', '{refund_amount}', '{refund_reason}'));
                break;

            case 'customer':
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{dollars_spent_order}', '{dollars_spent_total}', '{number_orders}', '{last_purchase_date}', '{item_names}', '{item_categories}'));
                break;

            case 'reminder':
            case 'normal':
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{order_billing_address}', '{order_shipping_address}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{item_name}', '{item_names}', '{item_category}'));
                break;
        }
    ...
    public function email_replacements( $reps, $email_data, $email_order, $email_row ) {
        ...
        if ( $email_order->is_cart == 1 ) {
            ...
            $item_list  = '<ul>';
            $item_cats  = '<ul>';
            $cart       = get_user_meta( $email_order->user_id, '_woocommerce_persistent_cart', true );
            $items      = $cart['cart'];
            ...
            if ( in_array( $email_row->interval_type, array('refund_manual', 'refund_successful', 'refund_failed') ) ) {
            ...
            if ( isset($meta['refund_amount']) )
                $refund_amount = woocommerce_price($meta['refund_amount']);
        ...
        if ( $email_type == 'generic' ) {
            ...
            if ( !empty($email_order->product_id) ) {
                $item   = WC_FUE_Compatibility::wc_get_product( $email_order->product_id );
    ...
    public function email_test_variables( $vars ) {
        ...
        switch ( $type ) {
            case 'generic':
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{order_billing_address}', '{order_shipping_address}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{item_names}', '{item_categories}'));
                break;
            ...
            default:
                $vars = array_merge($vars, array('{order_number}', '{order_date}', '{order_datetime}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{item_names}', '{item_categories}', '{dollars_spent_order}', '{dollars_spent_total}', '{number_orders}', '{last_purchase_date}', '{item_name}', '{item_category}'));
                break;
        }
    ...
    public function email_test_replacements( $reps ) {
        ...
        $order_date     = '';
        $order_datetime = '';
        $order_id       = isset($post['order_id']) ? trim( $post['order_id'] ) : '';
        ...
        if ( $email_type == 'generic' ) {
            ...
            if (! empty($order_id) ) {
                ...
                $billing_address    = '123 Main St., Bend, OR 97702';
                $shipping_address   = '123 Main St., Bend, OR 97702';
            ...
            $order_id           = (! empty($order_id) ) ? $order_id : '01100';
            ...
            if ( $order_id != '01100' && $order = WC_FUE_Compatibility::wc_get_order($order_id) ) {
                $order_id       = apply_filters( 'woocommerce_order_number', '#'.$order->id, $order );
            ...
            if (isset($post['product_id']) && !empty($post['product_id'])) {
                $item       = WC_FUE_Compatibility::wc_get_product( $post['product_id'] );
                $cats       = get_the_terms($item->id, 'product_cat');
    ...
    public function manual_types() {
        ?><option value="users"><?php _e('All Users', 'follow_up_emails'); ?></option>
