...
class FUE_Bookings {
    ...
    private function create_email_order( $booking_id, $triggers = array() ) {
        ...
        $emails     = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}followup_emails WHERE `interval_type` IN ($triggers_str) AND status = ". FUE::STATUS_ACTIVE );
