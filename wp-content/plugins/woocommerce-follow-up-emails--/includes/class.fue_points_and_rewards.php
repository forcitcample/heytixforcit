...
class FUE_Points_And_Rewards {
    ...
    public function after_points_increased( $user_id, $points, $event_type, $data = null, $order_id = 0 ) {
        ...
        $emails = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}followup_emails WHERE `interval_type` IN ('points_earned', 'points_greater_than') AND status = ". FUE::STATUS_ACTIVE );
    ...
    public function replace_variables( $text, $email_order ) {
        ...
        $search         = array( '{order_number}', '{order_date}', '{order_datetime}', '{customer_username}', '{customer_first_name}', '{customer_name}', '{customer_email}', '{points_earned}', '{reward_event_description}' );
        ...
        if (! $user ) {
            ...
            } elseif ( $event_data['user_id'] > 0 ) {
                $user = new WP_User( $event_data['user_id'] );
                $username = $user->user_login;
            } else {
                $username = '';
            }
        ...
        $replacements   = array( $order_number, $order_date, $order_datetime, $username, $first_name, $name, $email, $points, $description );
