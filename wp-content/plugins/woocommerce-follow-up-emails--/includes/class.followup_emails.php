...
class FollowUpEmails {
    ...
    public function __construct() {
        ...
        if ( get_option( 'fue_install_cron' ) ) {
            add_action( 'plugins_loaded', 'FollowUpEmails::install_cronjobs', 100 );
        }
        ...
        self::$scheduling_system = get_option( 'fue_scheduling_system', 'action-scheduler' );
    ...
    public static function update_db( $force = false ) {
        ...
        // install scheduled tasks on the next page load
        update_option( 'fue_install_cron', true );
...
KEY category_id (category_id)
        ...
        // remove optimize-tables cron
        $posts = $wpdb->get_results("SELECT ID FROM {$wpdb->posts} WHERE post_type = 'scheduled-action' AND post_title = 'sfn_optimize_tables'");
    ...
    public static function install() {
        ...
        // install scheduled tasks on the next page load
        // because action-scheduler is not available during activation
        //as it registers itself after the plugins_loaded action
        update_option( 'fue_install_cron', true );
    ...
    public static function install_cronjobs() {
        ...
        if ( function_exists('wc_next_scheduled_action') ) {
            wc_unschedule_action( 'sfn_optimize_tables', array(), 'fue' );
        ...
        delete_option( 'fue_install_cron' );
    ...
    public static function ajax_archive() {
        ...
        $id     = $_POST['id'];
        $resp   = array('ack' => 'OK');
        $type   = $wpdb->get_var( $wpdb->prepare( "SELECT email_type FROM {$wpdb->prefix}followup_emails WHERE id = %d", $id ) );

        // deactivate
        FUE::update_email_status( $id, FUE::STATUS_ARCHIVED );
    ...
    public static function ajax_unarchive() {
        global $wpdb;
        $id     = $_POST['id'];
        ...
        // activate
        FUE::update_email_status( $id, FUE::STATUS_ACTIVE );
        ...
        $resp['status_html'] = __('Active', 'follow_up_emails') .'<br/><small><a href="#" class="toggle-activation" data-id="'. $id .'">'. __('Deactivate', 'follow_up_emails') .'</a></small>
                                |
                                <small><a href="#" class="archive-email" data-id="'. $id .'" data-key="'. $email->email_type .'">'. __('Archive', 'follow_up_emails') .'</a></small>';
    ...
    public static function ajax_toggle_status() {
        ...
        if ($status == 0 || $status == -1) {
            ...
            // deactivate
            FUE::update_email_status( $id, FUE::STATUS_INACTIVE );
    ...
    private static function action_scheduler_import( $pos = 0, $length = 0 ) {
        global $wpdb;
