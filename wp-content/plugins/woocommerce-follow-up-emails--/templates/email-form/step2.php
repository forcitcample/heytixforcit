<div class="wrap email-form">
    ...
    <form action="admin-post.php" method="post" id="fue_form" class="fue-form-step2">
        ...
        <div class="field">
            ...
            <?php do_action('fue_email_form_before_subject', $defaults); ?>
            ...
            <div>
                ...
                <span class="hide-if-date non-signup interval_type_span hideable">
                    ...
                    <select name="interval_type" id="interval_type" class="interval_type hideable">
                        ...
                        ?>
                        <option
                            class="interval_type_option interval_type_<?php echo $key; ?> <?php if ( $key != 'purchase' && $key != 'completed' ) echo 'non-reminder'; ?> hideable <?php do_action('fue_form_interval_type', $key); ?>"
                ...
                <span class="show-if-date interval_date_span hideable">
                    ...
                    <select name="send_date_minute">
                        ...
                        <?php
                        for ( $x = 0; $x <= 55; $x+=5 ):
                            $sel = ((int)$defaults['send_date_minute'] === $x) ? 'selected' : '';
                        ?>
                        <option value="<?php echo $x; ?>" <?php echo $sel; ?>><?php echo $x; ?></option>
                        <?php endfor; ?>
                    </select>
                    <?php
                    $ampm = (isset($defaults['meta']['send_date_ampm'])) ? $defaults['meta']['send_date_ampm'] : 'am';
                    ?>
        ...
        <?php if ( isset($_GET['mode']) && $_GET['mode'] == 'quick' ): ?>
            <div class="field">
                <input type="hidden" name="action" value="sfn_followup_form" />
                <input type="hidden" name="step" value="2" />
                <input type="hidden" name="id" id="id" value="<?php echo $defaults['id']; ?>" />
                <input type="hidden" name="mode" value="quicksave" />
                <input type="submit" class="button-primary" id="save" name="save" value="<?php _e('Save', 'follow_up_emails'); ?>" />
            </div>
        <?php else: ?>
            <div class="field">
