...
jQuery(document).ready(function($) {
    ...
    $("#fue-email-subscriptions").on("change", "#subscription_product_id, #include_variations", function() {
        $("#fue-email-subscriptions").block({ message: null, overlayCSS: { background: '#fff url('+ FUE.ajax_loader +') no-repeat center', opacity: 0.6 } });
        var args = {
        ...
        $.post( ajaxurl, args, function( resp ) {
            ...
            $("#fue-email-subscriptions").unblock();
        }, 'json');
