...
jQuery( function ( $ ) {
    ...
    $("#titlewrap").prepend('<label for="title" class="fue-label">Email name</label>');

    // inject the content dummy inside the #postdivrich div
    $("#postdivrich").prepend( $("#email_content_dummy") );

    $("#email_content_dummy").css( {
        width: $("#content").width() - 100,
        top: $("#content").offset().top
    } );

    var content_dummy = function() {
        $("#email_content_dummy").click(function() {
    ...
    function fue_toggle_elements() {
        ...
        if (type == "signup") {
            var show = ['.interval_type_option', '.signup_description'];
            var hide = ['.settings_options', '.non-signup', '.always_send_tr', '.interval_type_span', '.adjust_date_tr', '.btn_send_save', '.email_receipient_tr', '.product_description_tr', '.product_tr', '.category_tr', '.use_custom_field_tr', '.interval_type_order_total_above', '.interval_type_order_total_below', '.interval_type_purchase_above_one', '.interval_type_after_last_purchase', '.interval_type_total_purchases', '.interval_type_total_orders', '.interval_type_after_last_purchase', '.interval_duration_date'];
        ...
        if (type == "customer") {
            var show = ['.always_send_tr', '.interval_type_order_total_above', '.interval_type_order_total_below', '.interval_type_purchase_above_one', '.interval_type_total_purchases', '.interval_type_total_orders', '.interval_type_total_purchases', '.interval_type_after_last_purchase', '.interval_type_span'];
            var hide = ['.adjust_date_tr', '.interval_type_option', '.always_send_tr', '.signup_description', '.product_description_tr', '.product_tr', '.category_tr', '.use_custom_field_tr', '.custom_field_tr', '.interval_duration_date'];
        ...
        jQuery("#send_coupon").change();
        jQuery("#tracking_on").change();
