jQuery( function ( $ ) {
    ...
    $("#fue-email-wootickets").on("change", "#wootickets_type", function() {
        var type = $(this).val();
...
jQuery(document).ready(function() {
    ...
    jQuery(":input.ajax_select2_products_and_variations").change(function() {
        if ( jQuery(this).val() ) {
...
jQuery(window).load(function() {
    ...
    if ( $("#wootickets_type") ) {
        $("#wootickets_type").change(function() {
            $(".product_tr, .category_tr, .excluded_category_tr").hide();

            switch ( $(this).val() ) {

                case 'all':
                    $(".ticket_product_tr, .ticket_category_tr").hide();
